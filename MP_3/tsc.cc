...
class Client : public IClient
{
    public:
        Client(const std::string& hname,
               ...
               const std::string& p)
            :hostname(hname), username(username), port(p), server_ip(""), server_port("")
            {}
        void getMaster();
    protected:
    ...
    private:
        ...
        std::unique_ptr<SNSService::Stub> cdn_stub_;
        std::unique_ptr<SNSService::Stub> serv_stub_;
...
int main(int argc, char** argv) {
    ...
    while ((opt = getopt(argc, argv, "h:p:i:")) != -1){
        switch(opt) {
            ...
            case 'p':
                cdn_port = optarg;break;
            default:
                std::cerr << "Invalid Command Line Argument\n";
        }
    }

    Client myc(hostname, username, cdn_port);
...
int Client::connectTo()
{
    ...
    serv_stub_ = std::unique_ptr<SNSService::Stub>(SNSService::NewStub(
               grpc::CreateChannel(
...
IReply Client::List() {
    ...
    Status status = serv_stub_->List(&context, request, &list_reply);
    IReply ire;
    ire.grpc_status = status;
...
IReply Client::Follow(const std::string& username2) {
    ...
    Status status = serv_stub_->Follow(&context, request, &reply);
...
IReply Client::UnFollow(const std::string& username2) {
    ...
    Status status = serv_stub_->UnFollow(&context, request, &reply);
    IReply ire;
    ire.grpc_status = status;
...
IReply Client::Login() {
    ...
    Status status = serv_stub_->Login(&context, request, &reply);
...
void Client::getMaster(){
    
    // connect to coordinator
    // create coordinator stub
    std::string login_info = hostname + ":" + port;
    cdn_stub_ = std::unique_ptr<SNSService::Stub>(SNSService::NewStub(
               grpc::CreateChannel(
                    login_info, grpc::InsecureChannelCredentials())));

    
    // call master stub function from coordinator using grpc
    ReqMaster request;
    request.set_id(username);
    RetMaster reply;
    ClientContext context;
    
    Status status = cdn_stub_->master(&context, request, &reply);
    ...
    string master_IP = reply.ip();
    string master_pno = reply.pno();
    
    hostname = reply.ip();
    port = reply.pno();
    
    cout << master_IP << endl << master_pno << endl;
